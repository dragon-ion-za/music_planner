/**
 * Tenant isolation tests — Property 14
 *
 * Creates two congregations with overlapping service dates and verifies that
 * ServiceRepository.findByCongregation() never returns rows from the other tenant.
 *
...
describe('Property 14 — ServiceRepository.findByCongregation returns only the requested tenant', () => {
  it('returns only services belonging to the requested congregation', async () => {
    const { cong: c1 } = await seedTenant('Tenant Church Alpha', [
      '2024-11-03', '2024-11-10', '2024-11-17',
    ]);
    await seedTenant('Tenant Church Beta', [
      '2024-11-03', '2024-11-10', '2024-11-17',
    ]);
    ...
    for (const svc of results) {
      expect(svc.congregationId).toBe(c1.id);
    }
  });
  ...
  it('returns zero services for a congregation that has none', async () => {
    ...
    const results = await ServiceRepository.findByCongregation(empty.id);
    expect(results).toHaveLength(0);
  });

  it('date range filter still excludes cross-tenant rows', async () => {
    const { cong: c1 } = await seedTenant('Tenant Church Gamma', [
      '2025-01-05', '2025-01-12',
    ]);
    await seedTenant('Tenant Church Delta', [
      '2025-01-05', '2025-01-12',
    ]);
