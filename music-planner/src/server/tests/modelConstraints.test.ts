/**
 * Model constraint tests — Properties 2–10
 *
 * Uses an in-memory SQLite database via testDb.ts.
 * No running PostgreSQL instance is required.
 *
 * Validates: Requirements 1.3, 2.2, 2.3, 3.4, 3.6, 4.2, 4.3, 5.2, 5.3
 */
...
describe('Property 3 — cascade delete removes all child rows', () => {
  it('deletes slot_templates, services, and service_slots when congregation is deleted', async () => {
    ...
    const templates = await SlotTemplate.findAll({ where: { congregationId: cong.id } });
    const services  = await Service.findAll({ where: { congregationId: cong.id } });
    const slots     = await ServiceSlot.findAll({ where: { serviceId: svc.id } });
...
// ─── Property 4: Slot template uniqueness ────────────────────────────────────
...
describe('Property 7 — song number uniqueness', () => {
  ...
  it('treats numbers as unique after uppercase normalisation', async () => {
    await makeSong('A2');
    // 'a2' normalises to 'A2' — should conflict
...
describe('Property 8 — translation_of_id SET NULL when target song is deleted', () => {
  it('sets translationOfId to NULL on songs pointing to the deleted song', async () => {
    ...
    const translation = await Song.create({
      number: 'A200',
      hymnal: 'A',
      translationOfId: target.id,
      familiarity: 0,
    });
